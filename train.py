...
def test(model, testloader, config, device):
    ...
    print('loss_degree_max', loss_degree_max, 'loss_degree_min', loss_degree_min, 'mid', mid, 'step1', step1, 'step2',
          step2)
    i = 0
...
def train(model = None, device=None,save_path='', config={}, train_dataloader=None, val_dataloader=None,group_index=1,index=1):
    ...
    for epoch in range(N_EPOCHS):
        ...
        f = open('./loss/valid_loss.txt', 'a')
        print('epoch:', epoch,'group_index,index',group_index,index, ' valid_loss:', valid_loss,'min_loss',min_loss, file=f)
        f.close()
