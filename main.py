...
class Main():
    def __init__(self, train_config, env_config, debug=False):
        ...
        enc_spatio = MMVAE.Encoder_Spatio(T, Graph_learner_n_hid, Graph_learner_n_head_dim, Graph_learner_head,
                      do_prob, device=self.device)
        enc_temporal = MMVAE.Encoder_Temporal(NUM_TASKS, LATENT_CODE_SIZE, factor=5, d_model=d_model, n_heads=8, e_layers=1, d_ff=d_model,
                 dropout=0, attn='full', activation='gelu',output_attention=False, distil=True,device=self.device)
        dec_rec = MMVAE.Decoder_Rec(NUM_TASKS, LATENT_CODE_SIZE, factor=5, d_model=d_model, n_heads=8, d_layers=1, d_ff=d_model,
                dropout=0, attn='full',activation='gelu',mix=True, c_out=1, device=self.device)
        dec_pre = MMVAE.Decoder_Pre(NUM_TASKS, LATENT_CODE_SIZE,factor=5, d_model=d_model, n_heads=8, d_layers=1, d_ff=d_model,
                 dropout=0, attn='full',activation='gelu', mix=True, c_out=1, device=self.device)
        sampling_z = MMVAE.Sampling_Temporal(NUM_TASKS, LATENT_CODE_SIZE, factor=5, d_model=d_model)
        gat = MMVAE.GAT(input_size=train_config['dim'] * train_config['slide_win'],
        ...
        self.model = MMVAE.MTVAE(enc_spatio, enc_temporal, sampling_z, gat, dec_rec, dec_pre, T, dec_in, d_model=d_model,topk_indices_ji=None)
